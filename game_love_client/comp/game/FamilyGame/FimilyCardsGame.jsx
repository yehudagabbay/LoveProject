// comp/game/FamilyGame/FamilyCardsGame.jsx
...
// אייקונים (אפשר להחליף לפי סגנון המשפחה)
...
const categoryName = (id) =>
  id === 1 ? 'היכרות משפחתית' : id === 2 ? 'כיף וצחוקים' : 'גיבוש משפחתי';
const levelName = (id) =>
...
const CATEGORY_THEME = {
  1: { name: 'Family Intro', bgColor: '#DCFCE7', icon: iconRelations },
  2: { name: 'Family Fun', bgColor: '#FEF3C7', icon: iconFun },
  3: { name: 'Family Bond', bgColor: '#E0E7FF', icon: iconLove },
};
...
export default function FamilyCardsGame({ route, navigation }) {
  const { cards = [], players: routePlayers = [] } = route?.params || {};

  // שמות השחקנים – במשחק משפחה אין ברירת מחדל, רק מה שהוזן במסך הבחירה
  const players = useMemo(
    () =>
      Array.isArray(routePlayers)
        ? routePlayers.filter((p) => p && p.trim().length > 0)
        : [],
    [routePlayers],
  );
  ...
  if (!players.length) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyText}>לא הוגדרו שחקנים למשחק.</Text>
        <TouchableOpacity
        ...
        >
          <Text style={styles.backButtonText}>⬅ חזרה לבחירת שחקנים</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.gameSubTitle}>
          זמן איכות משפחתי סביב קלפים ושאלות 🤍
        </Text>
      </View>
          ...
          {remainingCount > 0 ? (
            ...
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptySub}>
                חזרו לבחירת הקלפים כדי להתחיל משחק משפחתי חדש.
              </Text>
            </View>
          )}
        </View>
      ...
      <View style={styles.bottomHalf}>
        ...
        <View style={styles.controlsBox}>
          <View style={styles.controlsRow}>
            ...
            >
              <Text style={styles.btnTextDark}>בחירת קלפים</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        >
          ...
          >
            ...
            <ScrollView>
              ...
              >
                המשוב עוזר לנו לשפר את המשחק ולהוסיף משימות שמתאימות למשפחות
                כמוכם.
              </Text>
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  modalCardPhysicalBase: {
    ...
    padding: 10,
    elevation: 25,
    maxHeight: SCREEN_HEIGHT * 0.9, // ✅ לא יחתך במסכים נמוכים
  },
  modalCardInnerContent: {
    width: '100%',
    minHeight: SCREEN_HEIGHT * 0.75, // ✅ קלף גבוה
    maxHeight: SCREEN_HEIGHT * 0.88, // ✅ אבל לא יוצא מהמסך
    borderRadius: 22,
    padding: 25,
    justifyContent: 'space-between',
  },
  ...
  modalCardBodyScroll: {
    flex: 1,
    maxHeight: SCREEN_HEIGHT * 0.45, // ✅ מקום לטקסט + טיימר
  },
  modalScrollContent: {
    justifyContent: 'center',
    alignItems: 'center',
